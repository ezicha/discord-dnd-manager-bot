...
def build_access_overwrites(
    ...
    gm_only=True  -> участники кампании видят канал, но не могут писать
                      (а если это голосовой канал — не могут и подключаться сами),
                      писать/говорить может только ГМ.
    gm_only=False -> пишут и говорят все участники кампании.

    Для голосового канала с gm_only=True роль ГМа дополнительно получает
    Move Members на этом конкретном канале — участники не могут зайти сами,
    но ГМ может вручную переместить их туда (Discord это разрешает даже
    тем, у кого нет Connect в канале назначения, если у перемещающего есть
    Move Members). Без этого явного разрешения на канале у gm_role в принципе
    нет Move Members — роль создаётся ботом вообще без серверных прав,
    так что без этой строчки ГМ не смог бы затащить участника при всём желании.
    ...
    if gm_role:
        overwrites[gm_role] = discord.PermissionOverwrite(
            ...
            move_members=(True if (gm_only and is_voice) else None),
        )
